...
impl Default for ScoreboardCollection {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    pub fn get_objective_usage(&self) -> HashMap<String, f64> {
        ...
        counts
            ...
            .map(|(objective_name, (nonzero_count, total_entries))| {
                (objective_name.to_string(), *nonzero_count as f64 / *total_entries as f64)
            })
            .collect()
    }
    ...
    pub fn load_redis(domain: &str, con: &mut redis::Connection) -> anyhow::Result<Scoreboard> {
        ...
        for (_, player) in Player::get_redis_players(domain, con)?.iter_mut() {
            ...
            if let Some(scores) = &mut player.scores
                && let Some(playername) = &player.name
            {
            ...
            if let Some(nbt::Value::List(objectives)) = data.get("Objectives") {
                for objectives_entry in objectives.iter() {
                    if let nbt::Value::Compound(objective) = objectives_entry {
                        let name: String = if let Some(nbt::Value::String(name)) = objective.get("Name") {
                            name.to_string()
                        } else {
                        ...
                        let new_objective = Objective {
                            ...
                            display_name: if let Some(nbt::Value::String(display_name)) = objective.get("DisplayName") {
                                display_name.to_string()
                            } else {
                                bail!("Objective missing DisplayName")
                            },
                            render_type: if let Some(nbt::Value::String(render_type)) = objective.get("RenderType") {
            ...
            if let Some(nbt::Value::List(player_scores)) = data.get("PlayerScores") {
                for player_score_entry in player_scores.iter() {
                    if let nbt::Value::Compound(score_compound) = player_score_entry {
                        let name: String = if let Some(nbt::Value::String(name)) = score_compound.get("Name") {
                            name.to_string()
                        } else {
                            bail!("Score missing Name")
                        };
                        let objective: String =
                            if let Some(nbt::Value::String(objective)) = score_compound.get("Objective") {
                                objective.to_string()
                            } else {
                                bail!("Score missing Objective")
                            };
                        let score: i32 = if let Some(nbt::Value::Int(score)) = score_compound.get("Score") {
                            *score
                        } else {
                            bail!("Score missing Score")
                        };
                        let locked: bool = if let Some(nbt::Value::Byte(locked)) = score_compound.get("Locked") {
                            *locked != 0
                        } else {
                            bail!("Score missing Locked")
                        };

                        scoreboard.set_score(name, objective, Score { score, locked });
                    }
                }
            } else {
