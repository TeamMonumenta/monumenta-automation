    ...
    def add_replacements(self, replacements: [nbt.TagCompound], allow_unwanted_spawner_tags=False) -> None:
        ...
        for mob in replacements:
            ...
            if mob_name in self._mob_map[mob_id]:
                raise Exception("Mob {} '{}' already exists in replacements".format(mob_id, mob_name))

            # Make sure this mob doesn't match any of the existing substitutions
            for sub_matcher, sub_mob in self._substitutions:
                if sub_matcher(mob):
                    raise Exception("New replacements mob '{}' matches existing substitution '{}'".format(mob_name, get_entity_name_from_nbt(sub_mob)))
            ...
            for mob_id in self._mob_map:
                for mob_name in self._mob_map[mob_id]:
                    if mob_name == sub[0]:
                        ...
                        raise Exception("Substitution for '{}' also matches mob '{}'".format(sub[0], mob_name))

            if replacement is None:
                raise Exception("Substitution for '{}' missing replacement mob".format(sub[0]))
