...
def out_region_iter(region):
    """Export chests from a region file to json

    Returns a list of chest block info
    """
    ...
    for chunk in region.iter_chunks(min_x=min_x, min_y=min_y, min_z=min_z, max_x=max_x, max_y=max_y, max_z=max_z, autosave=False):
        for block_entity in chunk.recursive_iter_block_entities():
            if "chest" in block_entity.id:
                ...
                entry['nbt'] = block_entity.nbt.to_mojangson()
...
def in_region_iter(region):
    """Import chests from json to a region file

    Returns the number of chests imported
    """
    num_replaced = 0
    world_path = str(region.get_world_path())
    ...
    for chunk in region.iter_chunks(min_x=min_x, min_y=min_y, min_z=min_z, max_x=max_x, max_y=max_y, max_z=max_z, autosave=False):
        ...
        for block_entity in chunk.recursive_iter_block_entities():
            if "chest" in block_entity.id:
                chest_to_load = chests.get(f"{world_path} {block_entity.pos[0]} {block_entity.pos[1]} {block_entity.pos[2]}", None)
                if chest_to_load is not None:
                    # Update nbt and nothing else
                    block_entity.nbt.value = nbt.TagCompound.from_mojangson(chest_to_load["nbt"]).value
                    num_replaced += 1
                    changed = True
        if changed:
...
def export_chests(output_path, world_paths, min_x, min_y, min_z, max_x, max_y, max_z, num_threads):
    """Exports chests from a list of worlds to a json file."""
    out = []
    ...
    print(f"Wrote {len(out)} chests to {output_path}")

def import_chests(input_path, world_paths, min_x, min_y, min_z, max_x, max_y, max_z, num_threads):
    """Imports chests to a list of worlds from a json file."""
    # Build a map of all the chests that need updating
    # key = "world_path x y z"
    # value = { world/pos/nbt/name }
    global chests
    chests = {}
    num_to_replace = 0
    with open(input_path, 'r') as outfile:
        ...
        for element in data:
            if "chest" not in element["name"]:
                eprint("Warning: Refusing to update chest contained within an item: ", json.dumps(element))
            else:
                chests[f"{element['world_path']} {element['pos'][0]} {element['pos'][1]} {element['pos'][2]}"] = element
                num_to_replace += 1
    ...
    print(f"{num_replaced} / {num_to_replace} chests updated")



def main():
    """Argument handling for this chesti update tool"""
    multiprocessing.set_start_method("fork")

    arg_parser = argparse.ArgumentParser(description=__doc__)
    arg_parser.add_argument('world', type=Path, nargs='+')
    arg_parser.add_argument('--output', type=Path, nargs='?', help='Export chests to json')
    arg_parser.add_argument('--input', type=Path, nargs='?', help='Import chests from json')
    ...
    if output_mode:
        export_chests(output_path, world_paths, min_x, min_y, min_z, max_x, max_y, max_z, num_threads)

    else:
        import_chests(input_path, world_paths, min_x, min_y, min_z, max_x, max_y, max_z, num_threads)
