...
def out_region_iter(region, target_version):
    ...
    for chunk in region.iter_chunks(min_x=min_x, min_y=min_y, min_z=min_z, max_x=max_x, max_y=max_y, max_z=max_z, autosave=False):
        ...
        if chunk_version < target_version:
            ...
            cz = str(chunk.cz)
            chunk_contents = cx_contents.get(cz, dict())
            cx_contents[cz] = chunk_contents
            chunk_contents[type(region).__name__] = chunk.data_version
    ...
    for world_path in sorted(world_paths):
        ...
        for result in world.iter_regions_parallel(out_region_iter, err_func, num_processes=num_threads, read_only=True,
            ...
            for cx, cz_and_version in result.items():
                ...
                for cz, chunk_type_version_map in cz_and_version.items():
                    chunk_type_version_results = cz_version_map.get(cz, dict())
                    cz_version_map[cz] = chunk_type_version_results
                    for chunk_type, version in chunk_type_version_map.items():
                        chunk_type_version_results[chunk_type] = version
