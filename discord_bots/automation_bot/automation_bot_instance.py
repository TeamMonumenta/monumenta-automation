...
class AutomationBotInstance(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, rreact: dict):
        ...
        self._commands = {
            ...
            "gen server config": self.action_gen_server_config,
...
Performs the weekly update on the play server. Requires StopAndBackupAction.'''
        ...
        if min_phase <= 8 and config.COMMON_WEEKLY_UPDATE_TASKS:
            await self.display(ctx, "Updating shared domain files...")
            with open('/home/epic/3_DOMAIN_SHARED/zenithcharmlog.csv', 'w', encoding='utf-8') as fp:
                # Just blank it out, nothing to write
                pass
...
Easiest way to get this is putting an item in a chest, and looking at that chest and pressing f3+i
    ...
    async def action_gen_server_config(self, ctx: discord.ext.commands.Context, cmd, message: discord.Message):
        '''Generates server configuration (symlinks, config files) for one or more shards
Syntax:
`{cmdPrefix}gen server config valley isles orange`
`{cmdPrefix}gen server config --force valley isles orange`'''
        ...
        flags = []
        if config.K8S_NAMESPACE != "build":
            flags.append("--play")
        if "--force" in commandArgs:
            flags.append("--force")

        await self.display(ctx, f"Generating server config for [{' '.join(gen_shards)}]")
        await self.cd(ctx, self._server_dir)
        await self.run(ctx, [os.path.join(_top_level, "utility_code/gen_server_config.py")] + flags + gen_shards, displayOutput=True)
        await self.display(ctx, message.author.mention)
...
"""
    ...
    async def _internal_update_avatar(self, avatar_path: Path):
        ...
        user = self._bot.user
        if user is None:
            raise Exception('Bot has no user object; is it not signed in?')
