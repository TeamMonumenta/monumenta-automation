...
abbrev_node_to_geo_desc = {name: node.get("geoDescription", f"Unknown ({name})") for name, node in node_info.items()}
...
def run_helm_for_shard(namespace, shard):
    ...
    output_conf["geoDescription"] = abbrev_node_to_geo_desc.get(node, f"Unknown ({node})")
